...
function renderFrame() {
    ...
    const scale = 1.05 + bass / 600;
    const shiftX = Math.sin(time) * 10;
    const bright = 1 + bass / 350;
    const rot = Math.sin(time) * (bass / 200);

    if (visualLayer) {
        visualLayer.style.transform =
            `scale(${scale}) translateX(${shiftX}px) rotate(${rot}deg)`;
...
// ===============================
// SYNC LOGIC (NO HARD-CODED DURATIONS)
// ===============================
async function findSyncedTrack() {
    ...
    while (true) {
        ...
        if (elapsed < audio.duration) {
            return { index: idx, offset: elapsed };
        }
...
setInterval(() => {
    ...
    const expected = (Date.now() - GLOBAL_START_TIME) / 1000 % audio.duration;
